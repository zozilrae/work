...
코드 프로젝트가 아니라 **기술 문서 저장소**다. 산출물은 `docs/` 의 한국어 기술보고서(마크다운 + 동일 이름 PDF)이며, `tools/md2pdf.py` 는 그 PDF를 만들기 위한 단일 목적 스크립트다. 빌드·테스트·린트 설정은 없다.

- `docs/` — 보고서 원본(`.md`)과 산출 PDF. 둘 다 커밋한다.
- `tools/` — `md2pdf.py` 하나. 사용법과 폰트 선택 근거는 `tools/README.md` 에 있다.
- `notes/` — 작업용 메모 폴더(자리표시 README만 있음).
- `README.md` — 문서 목록 표 + 보고서별 개요. **문서를 추가/변경하면 반드시 함께 갱신한다.**

모든 문서·커밋 메시지는 한국어다. 본문은 경어 없이 `~이다 / ~한다` 평서체를 쓴다.

## PDF 생성

```bash
python3 tools/md2pdf.py docs/report.md                 # docs/report.pdf
python3 tools/md2pdf.py docs/report.md out.pdf --note "표지 하단 문구"
python3 tools/md2pdf.py docs/report.md --keep-html     # 중간 HTML 보존(레이아웃 디버깅)
```
...
한국어로 쓰고 `docs:` / `tools:` / `chore:` 접두사를 붙인다. 제목 한 줄 뒤에 본문으로 무엇이 들어갔는지 불릿으로 정리한다(기존 커밋 참조). 문서를 추가한 커밋은 해당 문서번호를 제목에 함께 적는다.
